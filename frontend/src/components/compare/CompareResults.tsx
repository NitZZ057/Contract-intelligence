...
function ModifiedClauseCard({ clause, index }: { clause: ModifiedClause; index: number }) {
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white shadow-sm">
      ...
      >
        <div>
          <p className="text-sm font-bold text-slate-900">{clause.clause_type}</p>
          <p className="mt-1 text-xs text-slate-500">AI-reviewed modification</p>
        </div>
        ...
        <div className="border-t border-slate-200 px-5 py-4">
          <div className="grid gap-4 xl:grid-cols-2">
            <div className="rounded-md border border-red-100 bg-red-50 p-4">
              <p className="text-xs font-bold uppercase tracking-wide text-red-700">Original</p>
              <p className="mt-3 text-sm leading-6 text-red-900 line-through decoration-red-400">{clause.original}</p>
            </div>
            <div className="rounded-md border border-green-100 bg-green-50 p-4">
              <p className="text-xs font-bold uppercase tracking-wide text-green-700">Updated</p>
              <p className="mt-3 text-sm leading-6 text-green-900">{clause.updated}</p>
            </div>
          </div>
          <div className="mt-4 rounded-md border border-amber-100 bg-amber-50 p-4">
            ...
            <p className="mt-2 text-sm leading-6 text-amber-900">{clause.explanation}</p>
          </div>
        </div>
      ) : null}
    </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        ...
        <section className="rounded-lg border border-slate-200 border-l-4 border-l-amber-500 bg-white p-5 shadow-sm">
          <h2 className="text-sm font-bold text-slate-900">Modified Clauses</h2>
          <div className="mt-4 space-y-3">
            {result.modified_clauses.length > 0 ? (
              ...
              <p className="text-sm text-slate-500">No modified clauses detected.</p>
            )}
          </div>
        </section>
      </div>
